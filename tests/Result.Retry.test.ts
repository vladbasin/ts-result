...
describe(".Retry()", () => {
    describe.each([
    ...
    ])("failFirstCalls: $failFirstCalls, retries: $retries, delay: $delay, expectSuccess: $expectSuccess", ({ failFirstCalls, retries, delay, expectSuccess }) => {
        test("correctly retries", done => {
            ...
            let calls = 0;
            const targetFactory = () => Result.FromPromise(new Promise((resolve, reject) => {
                calls++;
                recordAttempt();
            ...
            executeResult(done, Result
                .Retry(retries, delay, targetFactory)
                .onFailure(error => {
                    if (expectSuccess) {
                ...
                () => {
                    ...
                    expect(new Date().getTime() - startTime).toBeGreaterThanOrEqual((expectedAttempts - 1) * delay);
                }
            );
        });
    });
