...
describe(".onSuccessWhenExecute()", () => {
    describe.each([
    ...
    ])("checks success: $success, condition: $condition, expectSuccess: $expectSuccess", ({ success, condition, expectSuccess }) => {
        test(`produces ${expectSuccess}`, done => {
            ...
            executeResult(done, result
                .onSuccessWhenExecute(_ => condition, _ => Result.Ok(1))
                .onFailure(error => {
